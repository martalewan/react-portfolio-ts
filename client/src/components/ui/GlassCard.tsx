...
const GlassCard = ({ children, className = "", glow = false }: GlassCardProps) => {
    return (
        <div className={`max-w-350 relative bg-(--color-bg-inverse-faint) backdrop-blur-xl border border-(--color-border-faint) rounded-xsm shadow-lg p-4 md:p-8 ${className}`}>
            {glow && (
